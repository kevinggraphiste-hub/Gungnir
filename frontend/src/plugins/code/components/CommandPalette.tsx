      ...
      <div onClick={e => e.stopPropagation()} style={{ width: 520, maxHeight: 420, background: 'var(--bg-secondary)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={handleKey}
            placeholder="Ouvrir un fichier..." style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', color: 'var(--text-primary)', fontSize: 'var(--font-base)', fontFamily: 'inherit' }} />
          <kbd style={{ fontSize: 'var(--font-2xs)', padding: '1px 6px', background: 'var(--bg-tertiary)', borderRadius: 3, border: '1px solid var(--border)', color: 'var(--text-muted)' }}>Esc</kbd>
        </div>
        <div style={{ maxHeight: 340, overflow: 'auto' }}>
          {filtered.length === 0
            ? <div style={{ padding: 20, textAlign: 'center', fontSize: 'var(--font-sm)', color: 'var(--text-muted)' }}>Aucun fichier trouve</div>
            : filtered.map((f, i) => (
              <div key={f.path} onClick={() => select(f)} style={{
                display: 'flex', alignItems: 'center', gap: 10, padding: '8px 16px', cursor: 'pointer', fontSize: 'var(--font-sm)',
                background: i === selected ? 'var(--bg-tertiary)' : 'transparent',
                borderLeft: i === selected ? '3px solid var(--scarlet)' : '3px solid transparent',
              }}>
                <span style={{ fontSize: 'var(--font-md)', width: 18, textAlign: 'center', flexShrink: 0 }}>{FI[f.ext] || '\u{1F4C4}'}</span>
                <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{f.name}</span>
                <span style={{ flex: 1, color: 'var(--text-muted)', fontSize: 'var(--font-xs)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.path}</span>
